...
marker_list = as.list(unstack(rev(markers)))
```
...
```{r}
#changing sample names in combined.sct from sample_LXXXX_kb to LXXXX // can modify to put timepoint instead
...
# just testing out 
DefaultAssay(combined.sct) <- "integrated"
FeaturePlot(combined.sct, features = marker_list[[8]])
FeaturePlot(combined.sct, features = "Sox11", split.by = "orig.ident", combine = FALSE) %>% CombinePlots(ncol=3)
