...
Of the genes that were detected for both pipelines `r round(kb_greater_sig / (kb_greater_sig + cr_greater_sig) *100,digits = 2)`% of them had higher counts detected in Kallisto, for protein coding genes `r round(pc_kb_greater_sig / (pc_kb_greater_sig + pc_cr_greater_sig)*100,digits = 2)`% had higher counts in Kallisto and for lncRNAs `r round(lncRNA_kb_greater_sig / (lncRNA_kb_greater_sig + lncRNA_cr_greater_sig)*100,digits = 2)`% had higher counts in Kallisto.
...
```{r Heatmap}
kb_only_mat = kb_only_olf %>%
  dplyr::select(!Gene_name) %>%
  dplyr::select(!Gene_family) %>%
  as.matrix()

kb_annotation = kb_only_olf %>%
  dplyr::select(Gene_family) %>%
  as.matrix()

rownames(kb_annotation) = kb_only_olf$Gene_name

rownames(kb_only_mat) = kb_only_olf$Gene_name
...
col_fun = colorRamp2(c(0, 30,100), c("#219168", "white" ,"#b52d1b"))
...
Heatmap(kb_only_mat,
        ...
        column_title = "Total Counts Ordered By Timepoint")
```
