...
```{r}
#Collapse samples, sort by count twice, plot
...
cr_sums <- as.matrix(rowSums(collapsed_counts_cr, na.rm = TRUE))
kb_sums <- as.matrix(rowSums(collapsed_counts_kb, na.rm = TRUE))


cr_kb_sums <- cbind(cr_sums, kb_sums) %>% as_tibble(rownames = "Gene_name") %>% rename("Cell_Ranger" = V1, "kallisto" = V2)

pcoding_bm <- bm_tib %>% filter(Gene_Type == "protein_coding") 

notsame <- filter(cr_kb_sums, kallisto != Cell_Ranger) %>% mutate("Difference_kb_cr" = kallisto - Cell_Ranger) %>% filter(abs(Difference_kb_cr) > 50) %>% filter(Gene_name %in% pcoding_bm$Gene_Name)


ggplot(notsame, aes(y = Gene_name, x = Difference_kb_cr)) + geom_bar(stat = "identity") 
...
#as.matrix(rowSums(seurat_obj_cr[[i]]))->m
