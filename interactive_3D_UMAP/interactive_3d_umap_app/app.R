...
server <- function(input, output, session) {
  ...
  observeEvent(input$file, {
    if(!is.null(input$file)) {
      removeTab(inputId = "tabs", target = "New Plot")
      appendTab(inputId = "tabs",
                tabPanel("New Plot", plotlyOutput("new_graph") %>% withSpinner(type = 4, size = 0.5))
      )
    }
    else {
      appendTab(inputId = "tabs",
  ...
  graphType_all <- reactive({
    if(input$graphType == "Gene Expression") {
      #show("gene")
      return(get(input$gene, data_all()))
    }
    else {
      #hide("gene")
      return(get("Identity", data_all()))
    }
  })
  ...
  output$graph <- renderPlotly({
      plot_ly(data = data_all(), 
              x = ~UMAP_1, y = ~UMAP_2, z = ~UMAP_3, 
              color = graphType_all(),
              colors = gradColors_all(),
              type = "scatter3d", 
              mode = "markers", 
              marker = list(size = 2), 
              text=~Identity,
  ...
  output$inter <- renderPlotly({
      plot_ly(data = data_int(), 
              x = ~UMAP_1, y = ~UMAP_2, z = ~UMAP_3, 
              color = graphType_int(),
              colors = gradColors_int(),
              type = "scatter3d", 
              mode = "markers", 
              marker = list(size = 2), 
              text=~Identity,
