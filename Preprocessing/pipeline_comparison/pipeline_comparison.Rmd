...
ggplot(pc_filt_counts %>% filter(abs(Difference_kb_cr) > 5), aes(y = reorder(Gene_name, Difference_kb_cr), x = Difference_kb_cr, fill=Higher_Signal)) + geom_bar(stat = "identity") + ylab("Protein Coding Genes") + xlab("Average Gene Counts Per Cell") +labs(fill="Pipeline")

ggplot(lncRNA_filt_counts %>% filter(abs(Difference_kb_cr) > 1), aes(y = reorder(Gene_name, Difference_kb_cr), x = Difference_kb_cr, fill=Higher_Signal)) + geom_bar(stat = "identity") + ylab("lncRNA Genes") + xlab("Average Gene Counts Per Cell") +labs(fill="Pipeline")
...
kb_greater_sig / (kb_greater_sig + cr_greater_sig) 
pc_kb_greater_sig / (pc_kb_greater_sig + pc_cr_greater_sig)
lncRNA_kb_greater_sig / (lncRNA_kb_greater_sig + lncRNA_cr_greater_sig)
